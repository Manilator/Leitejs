...
function getMoviesByGenre(message, args) {
  if (args.length < 1) {
    message.channel.send(
      "You have to give me a genre id. Use **!genres** to check all genres available."
    );
    return 0;
  }
      ...
      Axios.get("https://api.themoviedb.org/3/discover/movie", {
        ...
        (response) => {
          ...
          try {
            ...
            message.channel.send(
              "Invalid genre id. Use **!genres** to check all genres available."
            );
          }
        },
        (error) => {
